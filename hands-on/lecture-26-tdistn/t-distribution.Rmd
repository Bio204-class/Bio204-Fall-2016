...
ggplot(data.frame(x = breaks), aes(x = x)) + 
  stat_function(fun = dt, args = list(df = 2), xlim = c(-6,6), n = 200, 
                geom = "line", size = 1, color = "blue") + 
  stat_function(fun = dt, args = list(df = 4), xlim = c(-6,6), n = 200, 
                geom = "line", size = 1, color = "purple") + 
  stat_function(fun = dt, args = list(df = 30), xlim = c(-6,6), n = 200, 
                geom = "line", size = 1, color = "pink") +   
  stat_function(fun = dnorm, xlim = c(-6,6), n = 200, 
                geom = "line", size = 0.75, color = "red", alpha=0.75, linetype="dashed") +
...
plot.norm <- ggplot() + 
  geom_line(aes(x = breaks, y = norm.density)) + 
  labs(x = "X", y = "density", title = "Normal distribution")

plot.t <- ggplot() + 
  geom_line(aes(x = breaks, y = t.density.df5)) + 
  labs(x = "X", y = "density", title = "t-distribution, df = 5")
