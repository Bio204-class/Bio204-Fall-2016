...
To illustrate our linear regression calculations we'll use a data set from a study of African lions.  A study by Whitman et al. (2004) showed thte amount of black coloring on the nose of male lions increases with age, and suggested that this might be used to estimate the age of unknown lions.  The lion data set is available at this link , and the bivariate relationship between age and proportion of black pigmentation looks like this:
...
\[
MS_\text{regression} =\frac{SS_\text{residual}}{df_\text{residual}} 
\]
...
| Residual             | $SS_\text{residual}$ | n-2 | $\frac{SS_\text{residual}}{df_\text{residual}}$ |  |
...
We can further use the sum-of-square decomposition to understand the relative proportion of variance "explained" (accounted for) by the regression model.
...
and compare the test statistic to the t-distribution with $n-2$ degrees of freedom to estimate a p-value.
